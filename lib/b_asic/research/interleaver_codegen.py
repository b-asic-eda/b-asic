"""VHDL code generation for interleaver memories."""
...
def memory_based_storage_entity(
    ...
    common.write(f, 2, "-- Memory port I/O")
    read_ports: set[Port] = {
    ...
    for read_port in read_ports:
        ...
        common.write(f, 2, f"{port_name} : in std_logic_vector(WL-1 downto 0);")
    ...
    for idx, write_port in enumerate(write_ports):
        ...
        if idx == len(write_ports) - 1:
            common.write(f, 2, f"{port_name} : out std_logic_vector(WL-1 downto 0)")
        else:
            common.write(f, 2, f"{port_name} : out std_logic_vector(WL-1 downto 0);")

    # Write ending of the port header
    common.write(f, 1, ");")
    common.write(f, 0, f"end entity {entity_name};", end="\n\n")


def register_based_storage_entity(
    f: TextIO, entity_name: str, collection: ProcessCollection, word_length: int
) -> None:
    memory_based_storage_entity(f, entity_name, collection, word_length)


def memory_based_storage_architecture(
    f: TextIO,
    ...
    *,
    ...
    common.write(f, 1, "-- Schedule counter", start="\n")
    common.constant_declaration(
        f,
    ...
    common.write(f, 1, "-- Memory common.write address generation", start="")
    common.write(f, 1, "--", end="\n")

    # Extract all the common.write addresses
    ...
    common.write(f, 0, f"end architecture {architecture_name};", start="\n")


def register_based_storage_architecture(
    f: TextIO,
