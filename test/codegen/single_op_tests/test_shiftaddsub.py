...
@pytest.mark.parametrize("data_type,latency,is_add,shift,mul_j,test_cases", TEST_PARAMS)
...
async def shiftaddsub_test(dut):
    ...
    for i, (in0, in1, is_add, shift, mul_j, expected) in enumerate(test_cases, 1):
        ...
        if is_complex:
            ...
            assert actual_re == expected_re and actual_im == expected_im, (
                f"Test {i}: ({in0[0]}{op_str}(({in1[0]}{mul_j_str})>>{shift})) +j({in0[1]}{op_str}(({in1[1]}{mul_j_str})>>{shift})) = "
                f"({actual_re} + j{actual_im}), expected ({expected_re} + j{expected_im})"
            )
            cocotb.log.info(
                f"✓ Test {i}: ({in0[0]}{op_str}(({in1[0]}{mul_j_str})>>{shift})) +j({in0[1]}{op_str}(({in1[1]}{mul_j_str})>>{shift})) = "
            ...
            assert actual == expected, (
                f"Test {i}: {in0} {op_str} ({in1} >> {shift}) = {actual}, expected {expected}"
            )
