...
def test_r3bf_compile(tmp_path, arch_r3bf):
    runner = get_runner()
    dt = DataType(16, is_complex=True)
...
def test_r3bf_io_registers_compile(tmp_path, arch_r3bf):
    runner = get_runner()
    dt = DataType(16, is_complex=True)
    printer = VhdlPrinter(dt)
    printer.print(arch_r3bf, path=tmp_path, io_registers=True)
...
def test_r3bf_io_registers_simulate(tmp_path, arch_r3bf):
    runner = get_runner()
    dt = DataType(16, is_complex=True)
    printer = VhdlPrinter(dt)
    printer.print(arch_r3bf, path=tmp_path, io_registers=True)
    ...
    runner.test(
        hdl_toplevel="r3bf",
        test_module="test_r3bf",
        testcase="r3bf_io_registers_test",
    )


@cocotb_test()
async def r3bf_io_registers_test(dut):
    import cocotb
    from cocotb.triggers import Timer
    ...
    dut.input_0_in_re.value = 0
    dut.input_0_in_im.value = 4096
    # Wait 2 extra cycles due to I/O registers
    await Timer(2 + 4, "ns")
    assert dut.output_0_out_re.value == 8192
    assert dut.output_0_out_im.value == 0
    # Send in data 2 cycles earlier to account for I/O register latency
    dut.input_0_in_re.value = 0
    dut.input_0_in_im.value = 4096
    await Timer(2, "ns")
    assert dut.output_0_out_re.value == 8192
