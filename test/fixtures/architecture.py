...
def arch_addsub():
    in0 = Input()
    in1 = Input()
    op0 = AddSub(in0, in1, is_add=True, latency=1, execution_time=1)
    in2 = Input()
    in3 = Input()
    op1 = AddSub(in2, in3, is_add=False, latency=1, execution_time=1)
    out0 = Output(op0)
    out1 = Output(op1)
    sfg = SFG(inputs=[in0, in1, in2, in3], outputs=[out0, out1])
